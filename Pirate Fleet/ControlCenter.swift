    ...
    func placeItemsOnGrid(_ human: Human) {
      ...
      //  let smallShip = Ship(length: 2, location: GridLocation(x: 3, y: 4), isVertical: true, isWooden: false, hitTracker: HitTracker())
       ...
       // let mediumShip1 = Ship(length: 3, location: GridLocation(x: 0, y: 0), isVertical: false, isWooden: true, hitTracker: HitTracker())
        ...
        let mine1 = Mine(location: GridLocation(x: 6, y: 0), guaranteesHit: true, penaltyText: "Oh Shit! You Hit a Mine!")
        human.addMineToGrid(mine1)
        
        //let mine2 = Mine(location: GridLocation(x: 3, y: 3))
        let mine2 = Mine(location: GridLocation(x: 3, y: 3), guaranteesHit: true, penaltyText: "Oh Shit! You Hit a Mine!")
        human.addMineToGrid(mine2)
        
        //let seamonster1 = SeaMonster(location: GridLocation(x: 5, y: 6))
        let seamonster1 = SeaMonster(location: GridLocation(x: 5, y: 6), guaranteesHit: true, penaltyText: "Oh Shit! You Pissed off a Sea Monster!")
        human.addSeamonsterToGrid(seamonster1)
        
        //let seamonster2 = SeaMonster(location: GridLocation(x: 2, y: 2))
        let seamonster2 = SeaMonster(location: GridLocation(x: 2, y: 2), guaranteesHit: true, penaltyText: "Oh Shit! You Pissed off a Sea Monster!")
